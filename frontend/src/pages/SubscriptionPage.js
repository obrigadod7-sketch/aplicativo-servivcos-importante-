...
export default function SubscriptionPage() {
  ...
  const fetchStatus = async () => {
    try {
      const r = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/subscription/status`, {
  ...
  const startSubscription = async () => {
    setLoadingPix(true);
    try {
      const r = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/subscription/start`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (r.ok) {
  ...
  const confirmPayment = async () => {
    setConfirming(true);
    try {
      const r = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/subscription/confirm-payment`, {
        method: 'POST',
